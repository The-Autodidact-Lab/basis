...
Examples:
  ...
  python run_gaia2_mini_a2a_multi_agent.py \\
    ...
    if args.verbose:
        logger.info("Verbose mode: ENABLED (will show subprocess output)")
    if args.save_logs:
        logger.info(f"Logs will be saved to: {args.save_logs}")
    logger.info("=" * 80)
    logger.info("")
    ...
    if args.save_logs:
        ...
        logger.info(f"Logging subprocess output to: {log_file.absolute()}")
    ...
    try:
        ...
        # Set up subprocess I/O
        stdout_dest = subprocess.PIPE if (args.verbose or args.save_logs) else None
        stderr_dest = subprocess.STDOUT if (args.verbose or args.save_logs) else None
        
        # Run subprocess with real-time output if verbose
        if args.verbose or args.save_logs:
            ...
            for line in process.stdout:
                ...
                if args.verbose:
                    print(line)
                if args.save_logs and log_file_handle:
            ...
            if return_code != 0:
                ...
                logger.error("-" * 80)
                if args.save_logs:
                    logger.error(f"Full output saved to: {log_file.absolute()}")
                return return_code
        else:
            # Non-verbose mode - capture output for error reporting
            ...
            if return_code != 0:
                ...
                logger.error("=" * 80)
                if result.stdout:
                    logger.error("Subprocess output (last 100 lines):")
                    logger.error("-" * 80)
        ...
        if args.save_logs:
            logger.info(f"Logs saved to: {log_file.absolute()}")
        logger.info("=" * 80)
        return 0
